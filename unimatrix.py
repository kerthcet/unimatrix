...
parser.add_argument('-s',
                    ...
                    default=85,
                    type=int)
parser.add_argument('-t', '--time', help='time. See details below', type=int)
...
class Writer:
    ...
    def get_char(start_time):
        ...
        if time.time() - start_time > 8:
            num = randint(0, len(members) - 1)
            if member == '':
                member = members[num]
            return member[randint(0, len(member) - 1)]
    ...
    def draw(self, node):
        ...
        if node.n_type == 'writer':
            if not node.white and node.last_char:
                ...
                character = self.get_char(self.start_time)
            if node.white:
    ...
    def draw_flasher(self, flasher):
        ...
        x = flasher[1]
        try:
            self.screen.addstr(y, x, self.get_char(self.start_time),
                               color | attr)
        except curses.error:
            pass
